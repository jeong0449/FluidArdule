...
BLUETOOTH_APLAY_EXECUTABLE = "bluealsa-aplay"
...
BLUETOOTH_STATUS_POLL_INTERVAL_SEC = 1.0
...
def get_file_source_entries() -> list[dict]:
    ...
    entries.append({"type": "source", "name": "bluetooth", "display": "Bluetooth audio"})
    return entries
...
def file_source_select() -> None:
    ...
    if item["name"] == "bluetooth":
        start_bluetooth_audio_receiver()
        return
...
def _bluetoothctl_lines(*commands: str) -> tuple[int, str]:
    """Run a short bluetoothctl command sequence non-interactively."""
    if not commands:
        return 1, ""
    payload = "\n".join(str(cmd).strip() for cmd in commands if str(cmd).strip()) + "\n"
    try:
        proc = subprocess.run(
            ["bluetoothctl", "--timeout", "3"],
            input=payload,
            text=True,
...
def bluetooth_connected_device() -> tuple[str | None, str | None]:
    """Return (MAC, display name) for the currently connected device only.

    Trusted devices that are not connected are intentionally ignored. Bluetooth
    pairing/trust remains a Console/SSH concern; the player screen reports only
    the device that is actually connected now.
    """
    code, out = run_cmd(["bluetoothctl", "--timeout", "3", "devices", "Connected"])
    ...
    for line in out.splitlines():
        ...
        info_code, info_out = run_cmd(["bluetoothctl", "--timeout", "3", "info", mac])
        if info_code == 0 and info_out:
            alias = re.search(r"^\s*Alias:\s*(.+)$", info_out, flags=re.M)
            name = re.search(r"^\s*Name:\s*(.+)$", info_out, flags=re.M)
            if alias and alias.group(1).strip():
                return mac, alias.group(1).strip()
            if name and name.group(1).strip():
                return mac, name.group(1).strip()
        return mac, fallback_name or "Bluetooth device"
    return None, None


def bluetooth_streaming_active(mac: str | None) -> bool:
    """Best-effort BlueALSA streaming-state check.

    BlueALSA versions differ in CLI detail output. When an explicit running or
    active flag is unavailable, the UI safely remains at Connected.
    """
    if not mac:
        return False
    code, out = run_cmd(["bluealsa-cli", "list-pcms"])
    if code != 0 or not out:
        return False
    mac_compact = mac.replace(":", "_").lower()
    candidates = [line.strip() for line in out.splitlines() if mac.lower() in line.lower() or mac_compact in line.lower()]
    for pcm in candidates:
        info_code, info_out = run_cmd(["bluealsa-cli", "info", pcm])
        if info_code == 0 and re.search(r"^\s*(?:Running|Active):\s*(?:true|yes|1)\s*$", info_out, flags=re.I | re.M):
            return True
    return False


def periodic_bluetooth_status_poll() -> None:
    """Refresh the Bluetooth Audio screen only while receiver mode is active."""
    if state.player_proc_kind != "bluetooth" or state.ui_mode != "player":
        return
    now = time.time()
    if now - state.last_bluetooth_status_poll_time < BLUETOOTH_STATUS_POLL_INTERVAL_SEC:
        return
    state.last_bluetooth_status_poll_time = now

    mac, name = bluetooth_connected_device()
    new_path = name or "Waiting for device..."
    if mac:
        new_status = "Streaming" if bluetooth_streaming_active(mac) else "Connected"
    else:
        new_status = "Waiting"

    if new_path != state.player_path or new_status != state.player_status:
        state.player_path = new_path
        state.player_status = new_status
        mark_dirty("Bluetooth status")


def start_bluetooth_audio_receiver() -> None:
    """Enter exclusive Bluetooth A2DP receiver mode using BlueALSA.

    Pairing and trust management remain manual Console/SSH operations.
    This mode intentionally stops the internal synth engine and does not mix
    Bluetooth audio with live keyboard sound.
    """
    global player_proc
    send_ui_status("BUSY", force=True)
    show_modal_message("Starting Bluetooth...", "Waiting for paired device")
    ...
    try:
        ...
        clear_modal_message()
        mark_dirty("bluealsa-aplay missing")
        send_ui_status("READY", force=True)
        return
    except Exception as exc:
        player_proc = None
        ...
        restore_current_preset_after_engine_restart()
        clear_modal_message()
        mark_dirty(f"Bluetooth failed: {exc}")
        send_ui_status("READY", force=True)
        return

    state.player_path = "Waiting for device..."
    state.player_proc_kind = "bluetooth"
    state.player_paused = False
    state.player_status = "Waiting"
    state.player_stop_requested = False
    state.player_origin_dir = None
    state.player_return_mode = "file_source"
    state.player_radio_station_id = None
    state.ui_mode = "player"
    invalidate_full_display()
    set_play_led("ON")
    clear_modal_message()
    mark_dirty("Bluetooth receiver active")
    send_ui_status("READY", force=True)
...
class TFTDisplay:
    ...
    def _draw_player(self, draw):
        if state.player_proc_kind == "bluetooth" or state.player_return_mode == "file_source":
            ...
            draw.rounded_rectangle((12, 64, self.width - 12, self.height - 48), radius=12, fill=BOX_BG)
            name_text = ellipsize_text(name, self.font_menu, self.width - 64)
            name_box = draw.textbbox((0, 0), name_text, font=self.font_menu)
            name_w = name_box[2] - name_box[0]
            draw.text(((self.width - name_w) / 2, 118), name_text, font=self.font_menu, fill=FG)

            if status != "Waiting":
                status_box = draw.textbbox((0, 0), status, font=self.font_body)
                status_w = status_box[2] - status_box[0]
                status_fill = STATUS_GOOD if status in {"Connected", "Streaming"} else DIM
                draw.text(((self.width - status_w) / 2, 174), status, font=self.font_body, fill=status_fill)
            return
        ...
        if state.player_proc_kind == "bluetooth" or state.player_return_mode == "file_source":
            ...
            down_label = "-"
            right_label = "-"
        elif state.player_proc_kind == "radio":
        ...
        if state.player_proc_kind == "bluetooth" or state.player_return_mode == "file_source":
            sel_label = "-"
        elif state.player_status == "Stopped":
            sel_label = "PLAY"
        else:
...
def periodic_bluetooth_stop_return() -> None:
    """Return to Media Player sources after briefly showing Bluetooth Stopped."""
    if state.bluetooth_stop_return_at <= 0.0:
        return
    if time.time() < state.bluetooth_stop_return_at:
        return
    state.bluetooth_stop_return_at = 0.0
    return_player_to_browser("Back to sources")
...
def return_player_to_browser(event: str = "Back to list") -> None:
    ...
    elif state.player_return_mode == "file_source":
        state.ui_mode = "file_source"
        state.browser_index = clamp_index(state.browser_index, len(get_file_source_entries()))
    else:
        state.ui_mode = "file_browser"
    invalidate_full_display()
    state.player_status = "Stopped"
        ...
        if state.player_proc_kind == "bluetooth" or state.player_return_mode == "file_source":
            if btn == "LEFT":
                if state.player_status == "Stopped":
                    return
                stop_player_keep_player("Bluetooth stopped")
                state.bluetooth_stop_return_at = time.time() + 0.8
                return
            if btn in {"RIGHT", "SEL", "UP", "DOWN"}:
                return
        if state.usb_eject_confirm:
            if btn == "LEFT":
                state.usb_eject_confirm = False
...
def main() -> None:
    ...
    try:
        while state.running:
            ...
            poll_player_state()
            periodic_bluetooth_status_poll()
            periodic_bluetooth_stop_return()
            process_pending_yoshimi_preview()
